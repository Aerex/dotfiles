...
def get_pass(account):
    output = check_output('pass Email/' + account,
                          shell=True, encoding='utf-8').splitlines()[0]
    return output


def get_user(account):
    lines = check_output('pass Email/' + account,
...
def get_imap_port(account):
    ...
    for line in lines:
        if re.search('imap_port:', line):
            return int(line.replace('imap_port:', '').replace(' ', ''))
