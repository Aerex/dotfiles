...
class show_files_in_finder(Command):
    ...
    def execute(self):
        import subprocess
        OS = subprocess.check_output(["uname", "-s"]).strip('\n')
        if OS == 'Darwin':
...
class extracthere(Command):
    def execute(self):
        ...
        OS = self.fm.execute_command('uname -s')
...
class vg(Command):
    ...
    def execute(self):
        import subprocess
        args = self.rest(1)
        command="ag --nobreak --noheading {0} | fzf -0 -1 |  awk -F: '{{print $1}}'".format(args)
        ...
        if fzf.returncode == 0:
            ...
            if os.path.isdir(fzf_file):
                ...
                self.fm.execute_command("$EDITOR {}".format(fzf_file))

# fzf_locate
class fzf_ag(Command):
    """
    :fzf_ag

    Find a file using fzf and ag.
    ...
    def execute(self):
        import subprocess
        args = self.rest(1)
        command="ag --nobreak --noheading {0} | fzf -0 -1  ".format(args)
        fzf = self.fm.execute_command(command, stdout=subprocess.PIPE)
        stdout, stderr = fzf.communicate()
        if fzf.returncode == 0:
            fzf_file = os.path.abspath(stdout.decode('utf-8').rstrip('\n'))
...
class delete(Command):
    ...
    def execute(self):
        ...
        if confirm != 'never' and (confirm != 'multiple' or many_files):
            ...
            self.fm.execute_command("trash {}".format(self.getFriendlyString(files)))
            #self.fm.delete(files)

    def getFriendlyString(self, files): 
         return " ".join(str(_) for _ in files) if isinstance(files, list) else files
    ...
    def _question_callback(self, files, answer):
        ...
        if answer == 'y' or answer == 'Y':
            text = "{} {}".format(RM_FILE, self.getFriendlyString(files))
